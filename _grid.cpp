...
int16_t grid[1201 * 1201];
void make_grid()
{
    fstream ifs("data/N30W096.hgt",ifstream::binary|ifstream::in);
    for (int y=0; y<=1200; ++y) {
        for (int x=0; x<=1200; ++x) {
            ...
            int16_t word = l|(h<<8);
            grid[y*1201+x] = word;
            if ((x%60==0) && (y%60==0)) cout << grid[y*1201+x] << " ";
        }
        if (y%60==0) cout << endl;
    }
}
...
/*    glBegin(GL_POINTS);
    ...
    // draw grid in points
    glBegin(GL_POINTS);
    for (int z=1200; z>=0; --z) {
        for (int x=0; x<1201; ++x) {
            int val = grid[z*1201+x]/2;
            if (val<20)       glColor4fv(colorSet[0]);
            else if (val<70) glColor4fv(colorSet[1]);
            else if (val<120) glColor4fv(colorSet[2]);
            else if (val<170) glColor4fv(colorSet[3]);
            else glColor4fv(colorSet[4]);
            glVertex3f((x-600)/2, val,(z-600)/2);
        }
    }
    glEnd();
...
int main(int argc, char **argv)
{
    ...
    gluPerspective(/* field of view in degree */ 40.0,
                   /* aspect ratio */ 1.0,
                   /* Z near */ 0.5, /* Z far */ /*40.0f*/ 4000.0);
    glMatrixMode(GL_MODELVIEW);
    gluLookAt(0.0, 1000.0, 8.0, /* eye location */
